...
% Second, if the model-based forecast is correct then $E_{\bf R} [ \rho_{\tilde I (X), Y-\mu(X)} ] = 0$  and therefore $\rho_{\tilde I(X), Y-\mu(X)} = O(N^{-1})$; however, if the forecasts are not correctly specified then the data quality index is unlikely to inversely scale with population size.
 ...
 Unfortunately, the first term of the doubly robust estimator cannot be computed as covariate information is not collected on the entire population.  Here, we suppose the same asymptotic regime as in Section~\ref{app:asympderivations} and use the probability sample at the time~$t \in \{t_l^\prime\}$ to estimate this term:
 $$
...
Table~\ref{app:indianatable} summarizes the statewide random molecular tests conducted in Indiana between April 25--29, 2020 as well as an additional non-random sample of 898 individuals during May 2--3, 2020.

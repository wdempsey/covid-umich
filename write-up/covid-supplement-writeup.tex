...
The proposed method relies on the assumption $I_{j,t}^{(NR)} \indep Y_{j,t} \, | \, X_{j,t}$, i.e., the sampling indicator in the non-probabilistic survey is conditionally independent of the outcome given the covariates.  However, estimates of population quantities based on inverse-weighting will be biased in the presence of `unobserved confounding'.  We may hope that the bias


The purpose of this section is to introduce XXX,

We consider a
\begin{align*}
\logit \pi \left( I_{t} = 1  | x_{t}, u_{t} \right) &= h_t(x_t) + \alpha u \\
E[ Y | i_t, x_t, u_t] = l(i_t, x_t)
\end{align*}
for some function~$h_t$, $l_t$ that are allowed to depend on time. Re-arranging the terms we see that .  This can be
$$
E[ Y | i_t x_t, u_t ] = l(t,x)  + \frac{\delta}{\alpha} \left( \logit \pi \left( I_{t} = 1  | x_{t}, u_{t} \right) - h_t (x_t) \right)
$$
The key idea in XXXX is that positing a distribution on $P(I_t = 1 | x_y, u_t)$ directly allows one to circumvent the need to specify a distribution for~$U_t$, a highly non-trivial task.

The logit-linear model does not lead to a tractable sensitivy analysis.  Instead, we define a sensitivity model as:
\begin{equation}
\begin{aligned}
\label{eq:sensmodel}
...
\end{aligned}
\end{equation}
The time-varying sensitivy parameter~$\alpha_t \in (0,1)$  controls the influence of the unobserved confounder~$U_t$ on selection propensity. In particular, $\alpha_t$ measure the change in belief of how likely an individual self-selects into the non-probabilistic sample:
$$
\alpha_t = E[ \pi_t (X_t,U_t) | I = 1] - E[ \pi_t (X_t,U_t) | I = 0].
$$
The sensitivity model satisfies the requirement that the self-selection propensity and conditional expectated outcome model can match the observed data:
\begin{align*}
P(I_t = 1 | X_t ) &= E[ E[ I_t | X_t, U_t ] | X_t ] = E[ \pi_t(X_t, U_t) | X_t ] = \pi_t (X_t) \\
E[Y_t = 1 | X_t, I_t = 1 ] &= E[ E[ Y_t | X_t, U_t, I_t =1 ] | X_t, I_t = 1 ] = Q(1,X) \\
\end{align*}
The sensitivity model is based on XXXX, and is adapted here to the non-probabilistic survey setting with potential time-varying confounding and a binary outcome.

\subsection{Bias}

By assumption, $X_t$ and $U_t$ together lead to consistent estimation via the IPW estimator:
\begin{align*}
...
\noindent {\bf IPW estimator:} Investigating the IPW estimator, we have:
\begin{align*}
...
\end{align*}
Using the fact that $\EE \left[ \pi_t (X_{J,t}, U_{J,t}) | X_{J,t} \right] = \pi_{t} (X_{J,t})$, we can see that the bias is given by
$$
\delta \EE \left[ \logit \pi_t (X_t, U_t) - \frac{\pi_t (X_{J,t}, U_{J,t})}{\pi_t (X_{J,t})} \logit \pi_t (X_t, U_t) \right].
$$
...
$$
\pi_t (X_t) \left[ \psi \left(\pi_t (X_t) \left(\frac{1-\alpha_t}{\alpha_t}\right) + 1\right) - \psi \left((1-\pi_t (X_t)) \left(\frac{1-\alpha_t}{\alpha_t}\right)+1\right) \right] + \frac{\alpha_t}{1-\alpha_t}\frac{\pi_t(X_t)}{1-\pi_t(X_t)}.
$$
...
$$
\delta \EE \left[\frac{\alpha_t}{1-\alpha_t}\left( \frac{2}{1-\pi_t(X_t)} - \frac{1}{\pi_t(X_t)} \right) \right].
$$
Note that this is an expectation over the population-level distribution of the covariate~$X$.  Thus, computation requires use of the
...
\begin{align*}
\delta \EE \bigg[ &\psi \left( \frac{1-\alpha_t}{\alpha_t} \pi_t(X_t) \right) - \psi \left( \frac{1-\alpha_t}{\alpha_t} \left( 1 - \pi_t(X_t) \right) \right) \\
&- \EE \bigg[ \psi \left( \frac{1-\alpha_t}{\alpha_t} \pi_t(X_t) + 1 \right) - \psi \left(\frac{1-\alpha_t}{\alpha_t} \left( 1 - \pi_t(X_t) \right) \right) \big | X_t, I_t = 1 \bigg] \bigg]
\end{align*}
...
$$
So we see that the bias is equal to the 2nd term in the bias under the IPW estimator.

\subsection{Reparametrization}

Following XX and XX, we reexpress the outcome-confounder strength in terms of the partial coefficient of determination
$$
R_Y^2 (\alpha, \delta) = \frac{\EE [( Y - Q(1,X) )^2 | I=1 ] - \EE[ (Y - \EE[ Y | X, U ])^2]}{E[(Y - Q(1,X))^2 | I = 1]}
$$
in terms of $\delta^2$.  Note the  To do so, write
\begin{align*}
...
By Theorem 5 in XXX, under the sensitivity model
$$
\alpha = 1 - \frac{\EE [ ]}{\EE[ ] }
$$

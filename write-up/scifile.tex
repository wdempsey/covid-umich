...
% Consider, for example, the state of New York.  Figure~\ref{fig:ny-covid-test} shows the number of tests and positive tests per day in New York state.  Figure~\ref{fig:ny-covid-frac} presents the fraction of tests that were positive per day in New York state.  For cancellation, the expected data qualities need to be uncorrelated and equal. Testing per day suggests it is plausible to assume $\Delta_t$ varies slowly over time.  Figure~\ref{fig:ny-covid-test} shows the number of tests has remained approximately constant recently relative to the NY population size.  Therefore, the assumption that $f_t$ varies slowly in time is reasonable. Finally, the assumption that the imperfect testing adjustment is constant is reasonable as no new testing procedures have been reported.

...
This paper has two goals.  The first is to express reservations at the use of case-counts as a proxy for disease prevalence and in estimation of standard epidemiological models for inference and forecasting.  The reason is straightforward: observed case-count data is plagued by selection bias and measurement error. Through simple calculations, we will demonstrate that the information gained from increasing testing capacity is limited in the presence of selection bias and when testing inaccuracies persist.  In particular, the millions of tests in the US have a small effective sample size when compared to random sampling.

The second is to show how probabilistic samples can also be used to derive a doubly robust approach that leverages epidemiological models along with pseudo survey weights built from combining nonprobability and probability samples.  Using a random sample from Indiana, we show that  for several key covariates and, given several assumptions, produce corrected estimates of the prevalence and true case-counts.  We end with several remarks on cautious use of these methods.
...
The prior section presented negative consequences of analysing the measurement-error adjusted prevalence from observed case-count data.  This raises the immediate question as to whether prevalence estimators can be improved using publicly available data.  In this section, we consider two directions. First, we use auxiliary information to build selection propensities and consider an inverse probability weighting (IWP) estimator.
We extend the statistical error decomposition to the weighted setting.
Second, we consider an extension of compartmental models from infectious disease epidemiology that adjust for selection bias and measurement-error as model-based forecasts of prevalence.
We then combine these epidemiological forecasts, selection propensities, and measurement-error corrections to generate \emph{doubly robust} prevalence estimators. We provide an appropriate extension of the statistical error decomposition and compare to the other prior decompositions. The proposed method and corresponding statistical error decompositions guide our recommendations in Section XX.

\subsection{Selection propensity estimation}

With non-probability samples, one can often reduce bias in two ways.  First, one may model the propensity for self-selection and use inverse probability weighting (IPW)~(cite: Elliott) techniques to adjust for selection bias.  In causal inference~(cite: Robins), given certain ignorability assumptions, the observational data permits direct estimation of treatment propensity.  In the COVID-19 context, one does not observe those who decide not to be tested.  In order to estimate selection propensities, one instead needs auxiliary information.  Here we present two methods for propensity estimation -- use of population-level summary statistics and use of a probability sample.

Let $X_j$ denote a vector of covariates for the $j$th individual in the population, $j=1,\ldots,N$.  Suppose the selection indicator $I_j$ is a Bernoulli random variable that depends only on these covariates, i.e., $P(I_j = 1 \mid X_j = x) = \pi (x; \theta)$.  Maximum likelihood estimation follows by maximizing
\begin{equation}
\label{eq:propensity}
...
The second sum is over the entire population.  Maximimum likelihood estimation therefore requires knowledge of covariate information for every individual in the population.  Typically, this is not possible.  We next present two options that are possible for analysis of selection propensities using only publically available COVID-19 case count data.

\subsubsection{Auxiliary information through population-level summary statistics}

For COVID-19 case-count data, the only covariate information provided by state and local governments are summary statistics for those factors discussed in Section XX -- age, sex, race, and (potentially) co-morbities.  These are discrete factors and therefore define a stratification of the population; then we can write $X_i \in \{1,\ldots,K\}$ to denote the individual belongs to strata $k$. One strata, for example, could be defined as anyone who is greater than or equal to 60 years of age, male, and white.
$$
...
$$
Taking the expecation of the second term with respect to the sampling design yields the second term in~\eqref{eq:propensity}.  Recall Indiana's probabilistic sample was a stratified random sampling design; therefore, the weights will depend on strata (i.e., 10 strata for each public health preparadness district).  Under simple random sampling and non-parametric selection propensity on a stratification variable, the weight is given by estimator of the $\hat \pi_k = (d_k / \tilde d_k) (n / N)$ where $\tilde d_k$ is the number of observations in strata $k$ in the random sample.

\subsubsection{An IPW estimator and statistical error decomposition.}
Given a selection propensity, define a weight~$W(x) = \pi (x; \hat \theta)^{-1}$. Then the IPW estimator adjusted for measurement error is given by
$$
\bar y_n^\star
= \frac{1}{1-FP-FN} \cdot \frac{\sum_{i=1}^n w(x_i) (y_i - FP)}{\pi (x_i; \hat \theta)}
\stackrel{(2)}{=} \frac{1}{1-FP-FN} \sum_{k=1}^K \frac{d_k}{N \pi_k} (\bar y_k - FP),
$$
where the second equality (2) is under the assumption that $x_i$ is a stratification variable with $k$ indexing the strata and $d_k$ is the number of samples in strata $k$. Under simple random sampling with one strata~$\hat \pi_k = n/N$ and we recover the estimator $\tilde y_n$ from Section~\ref{section:imperfecttesting}.

Let $\tilde I_j = I_j / (N \cdot \pi (X_j; \theta)) := I_j W_j$ for $j=1,\ldots,N$.  Then we can express $\bar y_n^\star$ as a weighted estimator $(1-FP-FN)^{-1} \sum_{i=1}^n $Then the error when comparing weighted estimator~$\bar y_n^\star$ to the true prevalence $\bar Y$ can be expressed as:
$$
\rho_{\tilde I, Y} \times \sqrt{\frac{1-f+ CV^2_W}{f}} \times \sigma_{Y} \times \underbrace{\left[ 1 - \Delta \times \frac{\bar Y}{1-\bar Y} \times \frac{FP(1-\bar Y) + FN \cdot \bar Y}{f_0 (1-\bar Y) + f_1 \bar Y} \right] \times \frac{1}{1-(FP+FN)}}_{D_M} \times \frac{E_J[\tilde I_J]}{N}
$$
...
Until now, this paper focused primarily on selection bias in coronavirus case-count data from a purely statistical perspective.  Here, we extend the approach to whether it is possible bring in simple models from infectious diease epidemiology.  Our goal is simply an estimate of $\bar y_k$, the prevalence in strata $k$.

We rely heavily on recently available software~\cite{Song2020} for estimation of such models.

We circle back and add in a SEIR model with measurement-error.  We can correct for selection bias two through simple rules (see Song and Mukherjee and others for more complex alternatives).  With such limited data we can only predict total counts and use the demographics as is to be representative.  This yields estimates at $\hat y_k$ for each strata.

\subsection{Doubly robust estimation}

$$
\sum_{k} P_k \hat y_k + \frac{1}{N} \sum_{i=1}^N I_i \frac{(Y_i - FP)/(1-FP-FN) - \hat y}{\pi_{K_i}}
$$
where $P_k$ is the population fraction in strata $k$. This estimator is ``doubly-robust'' in the sense that it is robust to either the SIR or the propensities being correctly specified.  Setting $\pi_i = n/N$ and $\hat y_k = 0$ yields the estimator $(\bar y_n - FP)/(1-FP-FN)$.
...
{\bf where to describe difference from Chen}.
...
$$
\left( P_k - \frac{n_k}{N \pi_k} \right) \hat y_k + \frac{n_k}{N \pi_k} \frac{\bar y_k - FP}{1-FP-FN}
$$

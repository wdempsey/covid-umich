...
Here we extend the IPW approach by~\cite{Chen2019} to this temporal setting by considering the joint likelihood
\begin{equation}
\label{eq:tvpropensity}
\sum_{t=1}^T \left[ \sum_{j=1}^N I_{j,t} \log \left( \frac{\pi_t (X_{j,t}; \theta)}{1-\pi(X_{j,t}; \theta)} \right) + \sum_{j=1}^N \log \left( 1 - \pi_t (X_{j,t}; \theta) \right) \right].
\end{equation}
{\bf $I_{j,t}$ will strongly depend on the selection history. So $X_{j,t}$ can contain features of past such as testing and symptom onset}


Suppose random samples are collected at each time $t =1,\ldots,T$.  Then the pseudo-likelihood can be re-written as in~\eqref{eq:auxinfoprob}, but with one term for each time point.  In practice, covariate information is available on the nonprobability sample at each time $t=1,\ldots,T$; however, random samples and therefore the necessary covariate information is typically only available at a subset of times. Here, we briefly discuss a non-parametric kernel approach.

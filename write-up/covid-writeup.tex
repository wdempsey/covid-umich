...
The prior section focused on the negative consequences.

The question is "What can be done".

Here we present a technique for combining epidemilogical forecasts, IPW built using auxiliary information, and measurement-error corrections to generate 'doubly robust' estimates of the prevalence (true case count).

We provide an appropriate extension of the statistical decomposition.
...
With non-probability samples, one can often reduce bias in two ways.  First, one may model the propensity for self-selection and use inverse probability weighting (IPW)~(cite: Elliott) techniques to adjust for selection bias.  In causal inference~(cite: Robins), given certain ignorability assumptions, the observational data permits direct estimation of treatment propensity.  In the COVID-19 context, one does not observe those who decide not to be tested.  In order to estimate selection propensities, one instead needs auxiliary information.  Here we present two methods for propensity estimation -- the first leveraging population-level summary statistics and the second leveraging the probability sample.
...
$$
Taking the expecation of the second term with respect to the sampling design yields the second term in~\eqref{eq:propensity}.  Recall Indiana's probabilistic sample was a stratified random sampling design; therefore, the weights will depend on strata (i.e., 10 strata for each public health preparadness district)

In the simple stratified case with a non-parametric selection propensity, the weight is given by estimator of the
quantity $\sum_k N \cdot \tilde d_k/n \log ( 1- \pi_k )$ where $\tilde d_k$ is the number of observations in strata $k$ in the random sample.
...
$$
\bar y_n^\star
= \frac{1}{1-FP-FN} \cdot \frac{1}{N} \sum_{i=1}^n \frac{y_i - FP}{\pi (x_i; \hat \theta)}
= \frac{1}{1-FP-FN} \sum_{k=1}^K \frac{d_k}{N \pi_k} (\bar y_k - FP).
$$
where $k$ indexes the strata and $d_k$ is the number of samples in strata $k$.
...
\subsection{Statistical decomposition of error}

For $i=1,\ldots,N$ let $\tilde Y_i = (Y_i - FP)/ (1- FP -FN)$ and $\tilde I_i = I_i / \pi_{K_i}$.  Then the error can now be expressed as:
$$
\rho_{\tilde I, Y} \times \sqrt{\frac{1-f+ CV^2_W}{f}} \sigma_{Y} \times D_M
$$
...
We circle back and add in a SEIR model with measurement-error.  We can correct for selection bias two through simple rules (see Song and Mukherjee and others for more complex alternatives).  With such limited data we can only predict total counts and use the demographics as is to be representative.  This yields estimates at $\hat y_k$
...
- Record symptom/asymptomatic status!

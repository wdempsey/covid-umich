...
Here, we consider covariate information that takes the form of a stratification variable with $K$ strata.  Based on these population-level dynamics, we can compute the number of \emph{new exposures} at time~$t$, i.e., $E^\new_{t} := - N \cdot (e_{t+1} - e_{t} + s_{t+1} - s_t)$ to denote the number of new exposures on day $t$ in the $k$th strata.  The joint distribution of strata-specific new exposures~$ \{ E^{\new}_{t,k} \}_{k=1}^K$ follows a multinomial distribution with
 $$
 ( E^\new_{1,t}, \ldots, E^\new_{K,t} ) \sim \text{Multinomial} \left( E^\new_t, (p_{1,t}, \ldots, p_{K, t}) \right).
 $$
 ...
 $$
 D_{k,r} \mid p, \theta, \nu \sim \text{Poisson} \left( \sum_{t=1}^r \text{IFR}_{k} \cdot E^\new_{k,t} \theta_{(r-t)} \right)
 $$
...
\end{remark}
 ...
 \begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{../figs/exposures_ppc.png}
 \caption{Posterior on daily new exposures.}
 \label{fig:casesppc}
 \end{subfigure}
 ...
 To check model fit, a posterior predictive check on aggregate death counts was performed. Figure~\ref{fig:deathsppc} shows the results which suggests chosen SEIR model fits the observed data well. Figure~\ref{fig:casesppc} presents the posterior distribution of total new exposures. As the active infection rate is defined as all infected individuals who’s viral load has yet to fall below detectable levels by RT-PCR testing, parameters underlying the population-level dynamics are combines with model-estimates of the number of newly exposed individuals to estimate the number of individuals with an active infection on each day. To compare with case count data, Figure~\ref{fig:undercounting} in the supplementary materials computes the weekly cumulative undercount factor of observed case counts.  Posterior distributions for the age-specific case counts are used as part of the doubly-robust estimator of the active infection rate.

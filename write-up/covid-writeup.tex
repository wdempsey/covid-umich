...
Coronavirus case-count data refers to the number of positive tests performed on each day.  Figure~\ref{fig:ny-covid} plots (dotted orange line) the number of reported confirmed COVID-19 cases per day in the state of Indiana.  Departments of Health in each state also typically report the total number of reported tests per day.  On August 5th, 2020, for example, there were 1075 total daily positive cases and 11,663 total daily tests.  Public databases maintained by \href{https://bit.ly/2UqFSuA}{Johns Hopkins University} and the \href{https://bit.ly/2vUHfrK}{New York Times} provide incoming county-level information of confirmed cases and deaths.
...
\subsection{Viral and antibody testing}
\label{section:testinginfo}

Here we briefly describe the difference between COVID-19 molecular and serological tests as well as the relationship between COVID-19 testing and the infection and immune response time frame. These distinctions will be useful later in the paper when discussing different random sample tests.

When an individual is infected with SARS-CoV-2, there will typically be an incubation period which may last between two to five days (cite: NPR) before the viral load is high enough to be detected.  A molecular test refers to tests that look increased viral load in the patient (such as RT-PCR or antigen tests).  These are tests such that if someone has an active infection, a positive result is expected after the incubation period.  A test with perfect sensitivity will come back positive for the next few weeks (cite: wlos).  At that point, the viral load will decrease and a molecular test with perfect specificity will come back negative.
...
While an individual may test positive on a molecular test for several weeks, it is important to note that the transmissibility o fth
This is particularly important for epidemiological forecasts build from compartmental models.


Recent systematic reviews have indicated, however, that  (cite:  NEJM)

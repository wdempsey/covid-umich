...
When presented with non-probability samples, one can often reduce bias in two ways.  First, one may model the propensity for self-selection and use inverse probability weighting (IPW)~(cite: Elliott) techniques to adjust for selection bias.  Unlike in causal inference~(cite: Robins) where the observational data permits direct estimation of propensities, one does not observe those who decide not to be  part of the .  Instead, we need auxiliary information.  Here, we leverage US census and the Indiana random testing demographic information to adjust the propensities.

First generate the counts per cell using raking on the marginal summary statistics.  Suppose the selection indicator could be modeled parametrically and depends only on the strata $P($

Start by using raking to get the ``number'' of individuals per strata.
\begin{rmk}[Raking]
All we get is testing demographic data.  Indiana provides.  Checked and it is often available upon digging.  Demographic data allows us to build stratified estimators.  We start by assuming that $d_k$ is the number of individuals in each strata is known.
\end{rmk}


Suppose we wanted to fit a logistic regression with the outcome being the selection indicator $Y_i$ given the strata information, coded as $X_i \in [K]$ for simplicity.
$$
\sum_{i=1}^N R_i \log \left( \pi (X_i; \theta) \right) + (1-R_i) \log \left( 1 - \pi (X_i; \theta) \right)
= \sum_{i=1}^N R_i \log \left( \frac{\pi (X_i; \theta)}{1-\pi(X_i; \theta)} \right) + \sum_{i=1}^N \log \left( 1 - \pi (X_i; \theta) \right)
$$
The first term involves quantities observed in the nonprobability sample and can be re-expressed as sums over the strata.  The second term, however, depends on the entire population.  We can re-write this as a sum over strata as well $\sum_{k=1}^K N_k \log \left( 1- \pi_k \right)$ where $N_k$ is the number of individuals in strata $k$.  The strata totals may be available from Census information. Alternatively, one could use the probability sample to construct a design-unbiased estimator of this quantity $\sum_k N \cdot \tilde d_k/n \log ( 1- \pi_k )$ where $\tilde d_k$ is the number of observations in strata $k$ in the random sample.  Under the assumptoin that $\pi_k < 1$, this leads to two potential estimators
$$
\hat \pi_k^{(1)} = \frac{d_k}{N_k}, \quad \hat \pi_k^{(2)} = \frac{d_k}{N} \frac{n}{\tilde d_k}.
$$
The assumptionWe start by assuming that $\pi_k < 1$ for all $k$.  This is not true (need to deal with very bad symptoms cases). We unfortunately don't have $\tilde d_k$ nor $d_k$, but we can try and get these into using standard techniques (raking).
...
$$
\frac{1}{1-FP-FN} \sum_{k} d_k (\tilde y - FP) / \pi_k.
$$

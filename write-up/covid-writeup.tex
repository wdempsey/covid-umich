...
Here, we take the suggested measures from~\cite{Woloshin2020} which report 70\% sensitivity is a reasonable estimates and~\cite{Cohen2020} which report 97.6\% specificity.  This corresponds to a false negative rate of $30\%$ and false positive rate of $2.4\%$.  To allow for potential uncertainty, we assume two \emph{synthetic} simple random samples.  The first is a simple random sample of true negative cases denoted $S_{FP}$; the second is simple random sample of true positive cases denoted $S_{FN}$.  As $\mu_t$ and $\theta_{t}$ are estimated independently at each time $t$, we can focus on the estimating equations per time point separately.

Here, we consider asymptotics under the following sequence (indexed by $L=1,2,\ldots$) of sampling regimes.  For $L >0$, consider $L$ random samples of size $n$ drawn at equally spaced times $\{ t^\prime_l \}_{l=1}^L$ over the study window~$[0,T]$.  Let~$\Delta_L$ denote the gap times between consecutive sample times.

For each~$L=1,2,\ldots$, the estimates of $\eta = (\mu_t, \theta_t, FP, FN)$ is given by:
$$
\Phi_L (\eta) = \left (
\begin{array}{c}
\frac{1}{N} \sum_{j=1}^N I_{j,t} \frac{Y_{j,t} - FP - (1-FP-FN) \cdot \mu_t}{\pi_{j,t}} \\
...
To prove consistency, first consider the setting where a random sample of size $n$ is observed at time $t$.  Let $\Phi_n (\eta)$ denote the corresponding estimating equations, which are equal to $\Phi_L(\eta)$ except for the second component which is given by
$$
\frac{1}{N} \left[ \sum_{j=1}^N I_{j,t} X_{j,t} - \frac{1}{N} \sum_{j = 1}^N \tilde I_{j,t} \tilde W_{j,t}  \pi_{j,t} X_{j,t} \right].
$$
Then under joint randomization of propensity score model and sampling designs, we have $E [ \Phi_n (\eta_0) ] = {\bf 0}$.  Consistency then follows by arguments in Section 3.2 of Tsiatis (2006). Under conditions C1-C6 from~\cite{Chen2019} and as $n$ increases, we have $\Phi_n (\hat \eta) = 0$ and $\Phi_n (\eta_0) = O_p ( n^{-1/2} )$.

To prove consistency under the sequence of sampling regimes, we simply need to show that $E[ \Phi_L (\eta_0) ] \to 0$ as $L$ increases.  First, we see that
$$
E \left[ \sum_{j = 1}^N \tilde I_{j,t} \tilde W_{j,t}  \pi_{j,t} X_{j,t} - \sum_{l=1}^L  \sum_{j = 1}^N \tilde K_{t,t^\prime_l} \tilde I_{j,t} \tilde W_{j,t}  \pi_{j,t} X_{j,t} \right]
$$

the additional assumption that $h_n \to 0$


By a first-order Taylor expansion we have $\hat \eta - \eta_0 = \left[ \phi_n (\eta) \right]^{-1} \Phi_n(\eta_0) + o_p (\tilde n^{-1/2})$ where $\phi_n(\eta) = \partial \Phi_n (\eta)/\partial \eta$.  To calculate
$$
...
Next, note that $\text{Var}(\Phi_n(\eta))$ can be decomposed into $A_{1,t} + A_{2,t} + A_{3,t} + A_{4,t}$

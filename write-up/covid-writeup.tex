...
Due to testing restrictions and other potential selection biases, publicly reported covid testing data may not be sufficient to understand active infection rates or the disease trajectory.  Here, we discuss two random samples that can be leveraged to get either unbiased estimates of the statewide active infection rate or as auxiliary information to address selection bias in the testing data.
...
\caption{Imperfect testing adjustment: relative frequency $f_1/f_0$ (x-axis) against odds ratio (y-axis) for $FP=0.005$ and $FN=0.172$. Color scaled so blue = $-10$, white = $0$, and red = $10$. \textcolor{red}{NEED TO DO FOR FP = 0.05 and FN = 0.3}}
\label{fig:heatmap}
\vspace{-0.3cm}
\end{figure}
...
Here we assume access to a probability sample measuring both the outcome and exact same set of covariates.  Let $\tilde I_j$ be an indicator that the individual was included in the probability sample and $\tilde W_j$ be the probability of inclusion for the $j$th individual (i.e., equal to $n/N$ for a simple random sample of size $n$). The probability sample is used to construct a design-unbiased estimator of the second term
\begin{equation}
...
\end{equation}
Taking the expectation of the second term with respect to the sampling design yields the second term in~\eqref{eq:propensity}.  This extends the work by~\cite{Chen2019} which considered auxiliary information coming only from a simple random sample.
...
\frac{\partial r_t}{\partial t} = - \gamma i_t.
\end{align*}
Following~\cite{Song2020}, we employ Runge-Kutta (RK4) approximations for discretization. Here, we consider covariate information to take the form of a stratificatoin variable $k$ with $K$ strata.  Let $\nu_{k,t}$ to denote the number of new infections on day $t$ in the $k$th strata.  AS there are a total of $N \cdot i_t := I_t$ new infections on day $t$, we assume the joint distribution of strata-specific new infection totals takes a multinomial distribution
$$
( \nu_{1,t}, \ldots, \nu_{K,t} ) \sim \text{Multinomial} \left( I_t, (p_{i1,t}, \ldots, p_{iK, t}) \right).
$$

Selection bias in case count data is addressed by bringing in a different source of information, COVID-19 death data.  Let $D_{k,r}$ denote the number of individuals who pass away on day $r$ from strata $k$.  Then
$$
D_{k,r} \mid p, \theta, \nu \sim \text{Poisson} \left( \sum_{t=1}^r p_{k} \nu_{k,t} \theta_{(r-t)} \right)
$$
where $p_{k}$ is the infection fatality rate for the $k$th strata.  This extends prior analysis of death data~\cite{Johndrow2020} by allowing the infection fatality rate to depend on age.  Indeed,~\cite{Levin2020} demonstrate that ``the estimated IFR is close to zero for children and younger adults but rises exponentially with age, reaching 0.4\% at age 55, 1.4\% at age 65, 4.6\% at age 75, and 15\% at age 85.''  See Appendix~\ref{app:ifr} for details on choice of $p_k$.
...
% For conciseness, parameter choices and inference are discussed in depth in Appendix~\ref{app:modelbased}.
...
$$
Using the constructed weights and the adjusted strata-level prevalence estimates, the IPW estimate under no measurement-error is $7.7$, a decrease of four percentage points from the observed prevalence of $11.7$. We also compute weights using Facebook's survey data collected in Indiana over that window of time but ignoring the household and prior testing information. The resulting IPW estimate under no measurement-error is $6.$

The study did not report sensitivity and specificity; therefore, we take the suggested measures from~\cite{Woloshin2020} which report 70\% sensitivity is a reasonable estimates and~\cite{Cohen2020} which report 97.6\% specificity.  This corresponds to a false negative rate of $30\%$ and false positive rate of $2.4\%$.  Therefore, the IPW estimates accounting for measurement-error are $5.6\%$ and $4.2\%$ respectively.  To account for uncertainty in
...
\subsection{Disease prevalence by April 2020}
...
The non-weighted estimate error is $8.6$\%.  Using~\eqref{eq:statdecomp}, we construct an estimate of the relative sampling rate as follows
\[
\rho D_M = \sqrt{\frac{f}{1-f}} \frac{\text{0.085}}{\sigma_Y} = 3.84 \times 10^{-2} \Rightarrow \Delta = 1.39 \times 10^{-2} \Rightarrow M = 6.1.
\]
For the IPW estimates, $3.8$\% and $2.4$\% respectively
\[
...
\]
For the IPW estimate using Facebook data, we have $\tilde M = 3.3$.

We can perform a sensitivity analysis by considering the range of false negative and false positive rates.  Here, we assume sensitivity between $81$\% and $91$\% and specificity between $95$\% and $98.8$\%~\cite{Katz2020}, which leads to a range for $M$ of $(5.60, 6.39)$ in the unweighted analysis.  Therefore, the impact on bias of $\hat R_t$ appears moderate.

This simple tool helps us understand how sensitivity and specificity impact biases and  therefore how much we should trust conclusions based on these assumptions.

...
In Section~\ref{section:improvedcasecount}, we present a doubly robust estimation method for combining epidemiological forecasts with inverse-probability weighting methods to estimate prevalence.  Weight estimation requires auxiliary information provided by random samples.  A statistical error decomposition illustrates a trade-off in use of weights: a potential increase in data quality but a reduction in data quantity.
...
When an individual is infected with SARS-CoV-2, there will typically be an incubation period which may last between two to five days (cite: NPR) before the viral load is high enough to be detected.  A \emph{molecular test} refers to tests that aim to detect increased viral load (e.g., RT-PCR or antigen tests).  If someone has an active infection, then after the incubation period a molecular test with perfect sensitivity will yield a positive result for the next few weeks (cite: wlos).  After that, the viral load will decrease and a molecular test with perfect specificity will come back negative.

While an infected individual may yield a positive molecular test for several weeks, they will likely stop transmitting the disease after the first few days after infection, meaning a positive molecular test does not imply transmissibility. This is of particular importance for epidemiological forecasts built from population-level data and impacts our model choice in Section~\ref{section:modelbased}.

Molecular tests are referred to as diagnostic tests.

Serological (or antibody) tests look for an immunological response to the virus.  A week or so after an individual is infected with SARS-CoV-2, the individual will start producing antibodies.  At this point, a serological test with perfect sensitivity will come back positive.  This test provides evidence of past infection while the molecular test provides evidence of an \emph{active} infection.


Seroprevalence studies are useful for disease surveillance.

To date, however, the number of seroprevalance surveys are limited.

See XX for further discussion.

Rapid and frequent molecular testing is cited as a key component (citEe: OECD and others) in effective strategies to identify active infections and prevent further spread.  ,

Significant logistic and capacity constraints have



Our primary goal is to estimate accurately estimate the fraction of active infections in the population.

This is the most relevant
Secondary goals include estimation of rates of change and the effective reproduction number which are also useful in policy making.


Alternatively,
Such testing would result in higher detection rates.
Will these results be reported to local authorities.
If not, selection bias in who uses these tests and then reports the results is likely to matter.


When an individual is infected with SARS-CoV-2 there is a

A viral test such as the RT-PCR
...
\subsection{COVID-19 case counts, viral testing counts, and deaths}
\label{section:testinginfo}

Coronavirus case-count data refers to the number of positive diagnostic tests performed on each day.  Figure~\ref{fig:ny-covid} plots (dotted orange line) the number of reported confirmed COVID-19 cases per day in the state of Indiana.  Departments of Health in each state also typically report the total number of reported tests per day.  On August 5th, 2020, for example, there were 1075 total daily positive cases and 11,663 total daily tests.  Public databases maintained by \href{https://bit.ly/2UqFSuA}{Johns Hopkins University} and the \href{https://bit.ly/2vUHfrK}{New York Times} provide incoming county-level information of confirmed cases and deaths.
...
Many states provide aggregate demographic information on those who requested a test as well as those who tested positive for SARS-CoV-2. Indiana's \href{https://www.coronavirus.in.gov/2393.htm}{COVID-19 dashboard}, for instance, provides statewide demographics for age, gender, ethnicity, and race.  The demographic information is based on cumulative testing.  Daily versions cannot be easily reconstructed from available public data.  In addition, the Regenstrief Institute provides additional comorbidity information among those who tested positive for COVID-19 in the state of Indiana, including hypertension, diabetes, and chronic obstructive pulmonary disease (COPD). These comprise the main set of covariate information available from public sources for Indiana. Similar demographic information is likely available for other states.co

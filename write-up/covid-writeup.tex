...
\begin{align*}
\frac{\partial s_t}{\partial t} &= - \beta s_t i_t; \quad \frac{\partial e_t}{\partial t} = \beta_t s_t i_t - \sigma e_t; \quad \\
\frac{\partial i^{(T)}_t}{\partial t} &= \sigma e_t - \gamma i_t; \quad
\frac{\partial r_t}{\partial t} = - \gamma i_t.
\end{align*}

We allow $\beta_t$ to be time-varying to account for the

"2020-03-23", "2020-06-15", "2020-10-01".

$w_t = 1/ \left( 1 + \exp \left( - \xi (t - t_1 - \nu)  \right) \right)$

We defined a weight $w^{(1)}_t = 1$ prior to time $t_1$ and goes to $1$ as $t \to \infty$

to accoun tfor

$\beta (1-w_t) + \eta weight$
...
\begin{figure}[!th]
\centering
 \includegraphics[width=.9\linewidth]{../figs/logifr.png}
 \caption{Logarithm of Infection Fatality Rate (IFR) by Age.  Linear model on the log-scale fits well and allows us to extrapolate publicly-reported IFR numbers to entire age range.}
 \label{fig:logifr}
\end{figure}

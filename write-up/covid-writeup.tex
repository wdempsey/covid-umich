...
Here, we present briefly a basic epidemiological state-space model -- the susceptible, infected, and removed (recovered and death) model, or SIR model.
The SIR model was originally proposed by (cite: Osthus, Forecasting seasonal influenza with a state-space sir model) with only one-dimensional time series of infected proportions. We refer the reader to Chapters 9-12 of [30] for a substantive introduction to state-space models.

More advanced versions of the SIR model can be used to make epidemiological forecasts.  Here, we focus on the SIER model due to availability of existing software.  We rely heavily on recently available software~\cite{Song2020} for estimation of such models.

The current prediction does not (A)
...
Two methods have been presented.  The first uses IPW to adjust for differential selection into the

Rather than relying on either independently, here we show how to combine these.
$$
\sum_{i=1}^N \mu (x_i; \hat \beta) + \frac{1}{\sum_{i=1}^n w(x_i)}
$$

There are again two options.  In the first,
$$

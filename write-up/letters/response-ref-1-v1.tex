...
\begin{enumerate}
	...
	We have no built a notation table and made the notation different.  Apologies for the confusion. We use $I_j^{R}$ and $I_j^{NR}$.  As stated in our answer to question set XX (subquestion XX) we have
	\vspace{5mm}
	...
	Section 4.1.3 allows researchers to choose to go beyond moving windowed averages (i.e., simply take weekly averages).  The kernel function allows the user to choose weighted averages that may be more robust than the uniform kernel (which is equivalent to the moving averages).
	\vspace{5mm}
\end{enumerate}

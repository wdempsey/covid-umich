...
library(lubridate)
## MAKE DATES
age_data$date = mdy(age_data$..date)
gender_data$date = mdy(gender_data$..date)
ethnicity_data$date = mdy(ethnicity_data$..date)
race_data$date = mdy(race_data$..date)
...
day = min(age_data$day)
year = 2020

construct_combos <- function(day, year) {
  addweight = 7/weekweight$day[weekweight$day == day]

  subset_age = age_data[age_data$day == day & age_data$year == year,]
  subset_gender = gender_data[gender_data$day == day & gender_data$year == year,]
  subset_ethnicity = ethnicity_data[ethnicity_data$day == day & ethnicity_data$year == year,]
  subset_race = race_data[race_data$day == day & race_data$year == year,]
...
daysin2020 = unique(age_data$day[age_data$year == 2020])
daysin2021 = unique(age_data$day[age_data$year == 2021])

dailycoviddata = construct_combos(daysin2020[1],2020)

for(day in daysin2020[-1]) {
  temp_dailydata = construct_combos(day,2020)
  dailycoviddata = rbind(dailycoviddata, temp_dailydata)
}

for(week in daysin2021) {
  temp_dailydata = construct_combos(day,2021)
  dailycoviddata = rbind(dailycoviddata, temp_dailydata)
}

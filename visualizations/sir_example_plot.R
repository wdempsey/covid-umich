...
lines(bias2_new$Rt, lty = 1, col = "grey50")
lines(bias2_new$Rt+bias2_new$logbias, lty = 2, col = "grey50")
axis(side =1, labels = FALSE)
...
lines(bias2$r+bias2$bias, col = "grey50", lty = 2, lwd = 2)
lines(bias3$r+bias3$bias, col = "grey50", lty = 3, lwd = 2)
axis(side =1, labels = FALSE)
...
png(filename = "../figs/sir_rt.png",
    width = 6.5, height = 4, units = "in", res = 1200, pointsize = 12)
par(mar = c(3,4,1,1)+0.1)
plot(bias1$Rt, type = "l", ylim = c(ymin, ymax), axes = FALSE, bty = "n", ylab = "")
lines(bias1$Rt + bias1$logbias, lty = 2)
abline(h = 1.0, col = "grey", lty = 2)
lines(bias2$Rt+bias2$logbias, lty = 2, col = "grey50", lwd = 2)
lines(bias3$Rt+bias3$logbias, lty = 3, col = "grey50", lwd = 2)
axis(side =1, labels = FALSE)
axis(side =2, cex.axis = 1)
mtext(expression(R[t]), side =2, line = 2, cex = 1.5)
mtext("Time", side = 1, line = 1, cex = 1.5)
legend(35, 0.9, legend = c("M=1", "M=2", "M=3", "M=4"), lty = c(1,2,2,3), col = c("black", "black", "grey50", "grey50"), cex = 1.5, bty = "n", lwd = c(1,1,2,2))
dev.off()
